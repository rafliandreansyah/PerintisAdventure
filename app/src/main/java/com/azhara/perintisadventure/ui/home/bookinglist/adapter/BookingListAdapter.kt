package com.azhara.perintisadventure.ui.home.bookinglist.adapter
...
class BookingListAdapter :
    ListAdapter<BookingList, BookingListAdapter.BookingListViewHolder>(DIFF_CALLBACK) {
    ...
    override fun onCreateViewHolder(
        ...
        return BookingListViewHolder(
            LayoutInflater.from(parent.context).inflate(R.layout.rv_item_listbooking, parent, false)
        )
    }
    ...
    inner class BookingListViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        fun bind(bookingList: BookingList?) {
            with(itemView) {
                ...
                tv_item_list_booking_total_price.text =
                    "Rp. ${decimalFormat(bookingList?.totalPrice)}"
                if (bookingList?.uploadProofPayment == false) {
                    tv_item_list_booking_status_payment.text = "Menunggu pembayaran"
                    tv_item_list_booking_status_payment.background =
                        ContextCompat.getDrawable(context, R.drawable.background_color_orange)
                }
                if (bookingList?.uploadProofPayment == true && bookingList.imgUrlProofPayment != null) {
                    ...
                    tv_item_list_booking_status_payment.background =
                        ContextCompat.getDrawable(context, R.drawable.background_color_yellow)
                }
                if (bookingList?.statusPayment == true) {
                    tv_item_list_booking_status_payment.text = "Pemesanan berhasil"
                    tv_item_list_booking_status_payment.background =
                        ContextCompat.getDrawable(context, R.drawable.background_color_green)
